...
标识符由一个字母开头（JS规范中还允许以下划线`_`和美元符`$`开头），其后可选择性地加上一个或多个字母，数字或下划线。
...
**处理不需要的属性：** 当你想让对象在运行时动态获自身信息时，关注更多的是数据，这时应该让你的程序做检查并丢弃掉值为函数的属性。使用`hasOwnProperty`方法可以检查对象是否拥有独有的属性，如果有则返回`true`，它不会检查原型链。
